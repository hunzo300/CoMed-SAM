...
class NpyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_4ch = np.load(os.path.join(self.img_path, img_name), "r")  # (3, H, W)
        img_4ch_resized = np.stack([resize(img_4ch[i], (1024, 1024), anti_aliasing=True) for i in range(3)])
        img_rgb_4 = np.stack([np.stack([img_4ch_resized[i]]*3, axis=0) for i in range(4)])  # (3, 3, 1024, 1024)

        if self.dropout:
            zero_indices = random.sample(range(4), random.randint(0, 2))
...
class CoMedSAM(nn.Module):
    ...
    def __init__(self, image_encoder_factory, mask_decoder, prompt_encoder, indicator,
        ...
        self.image_encoders = nn.ModuleList([self.image_encoder_factory() for _ in range(3)])

        self.conv1 = nn.Conv2d(256 * 3, 512, kernel_size=3, padding=1)
        self.conv2 = nn.Conv2d(512, 512, kernel_size=3, padding=1)
        self.conv3 = nn.Conv2d(512, 256, kernel_size=3, padding=1)
        self.act = nn.GELU()

        self.tr_proj_in = nn.Conv2d(256 * 3, d_model, kernel_size=1)
...
"--tr_npy_path",
type=str,
default="/CoMed-sam_dataset/Chaos/Chaos_npy_train_dataset_1024image",
help="path to training npy files; two subfolders: gts and imgs",
)
parser.add_argument(
"--val_npy_path",
type=str,
default="/CoMed-sam_dataset/Chaos/Chaos_npy_val_dataset_1024image",
help="path to training npy files; two subfolders: gts and imgs",
)
parser.add_argument(
